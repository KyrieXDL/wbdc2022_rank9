...
def eval(model, data_loader, get_pred_and_loss, compute_loss=True, eval_max_num=99999):
    ...
    model.eval()
    loss_l, emb_l, vid_l = [], [], []
    loss_mlm, loss_mfm, loss_itm = [], [], []
    loss_ml, loss_f, loss_i = 0.0, 0.0, 0.0
    ...
    return torch.mean(torch.stack(loss_l)), np.array(emb_l), loss_ml, loss_f, loss_i
...
#                     print('-->grad_value:',parms.grad)
                ...
                if step == 20 or (step % 1000 == 0 and step > 0):
                    elapsed_seconds = time.time() - start  # Evaluate the model on val_loader.

                    val_loss, emb, loss_m, loss_f, loss_itm = eval(
                    ...
                    logging.info(
                        ...
                        f"mfm_loss={loss_f}|itm_loss={loss_itm}|"
        ...
        torch.save(model.module.bert.state_dict(), args.savedmodel_path + f'/pytorch_model_{epoch}.bin')
        # model.load_state_dict(torch.load(model_path_epoch))  # Load best model
        # torch.save(model.state_dict(), model_path)
        val_loss, emb, loss_m, loss_f, loss_itm = eval(model, val_loader, get_pred_and_loss=get_pred_and_loss,
                                                       eval_max_num=99999)
        # label, spear = evaluate_emb_spearman(emb, vid_l, label_path=f"{DATA_PATH}/pairwise/label.tsv")
        logging.info(f"val_loss={val_loss:6.4}|mlm_loss={loss_m}|mfm_loss={loss_f}|itm_loss={loss_itm}")
...
def main(args):
    ...
    for fname in ['pretrain', 'model', 'data']:
        logging.info('=' * 66)
        with open(f'./pretrain/config/{fname}_cfg.py') as f:
    ...
    for fold in range(NUM_FOLDS):
        ...
        # print("加载权重")
        # model.load_state_dict(torch.load('./premodel_1/model_pretrain_1.pth'))  # Load best model
        # optimizer
...
def parse_args():
    ...
    parser.add_argument('--unlabeled_annotation', type=str, default=data_path + 'data/annotations/unlabeled_new.json')
    parser.add_argument('--unlabeled_zip_frames', type=str, default=data_path + 'data/zip_frames/unlabeled/')
    parser.add_argument('--unlabeled_zip_feats', type=str, default=data_path + 'data/zip_feats_unlabled/unlabeled.zip')
