...
class MultiModal_Pretrain(nn.Module):
    ...
    def forward(self, frame_feats, frame_feats_mask, text_input_ids, text_segment_ids, text_attention_mask, alpha=0):
        ...
        if 'itm' in self.config['tasks']:
            loss_itm = self.itm_forward(visual_emb, text_emb, visual_attn_mask, text_attn_mask, sim_i2t, sim_t2i)

        if 'mlm' in self.config['tasks'] and 'mfm' in self.config['tasks']:
            loss_mlm, loss_mfm = self.mlm_mfm_forward(frame_emb, frame_attn_mask, text_input_ids, text_attn_mask,
    ...
    def itm_forward(self, visual_emb, text_emb, visual_attn_mask, text_attn_mask, sim_i2t, sim_t2i):
        ...
        text_emb_all = torch.cat([text_emb, text_emb_neg, text_emb], dim=0)
        text_attn_mask_all = torch.cat([text_attn_mask, text_attn_mask_neg, text_attn_mask], dim=0)
        visual_emb_all = torch.cat([visual_emb, visual_emb, visual_emb_neg], dim=0)
        visual_attn_mask_all = torch.cat([visual_attn_mask, visual_attn_mask, visual_attn_mask_neg], dim=0)
        ...
        # image text match的ground truth，图像文本对的gt为1，在batch采样负样本生成的图像文本对的gt为0
        itm_labels = torch.cat([torch.ones(bs, dtype=torch.long), torch.zeros(2 * bs, dtype=torch.long)],
                               dim=0).to(visual_emb.device)
        loss_itm = F.cross_entropy(itm_output, itm_labels)
    ...
    def mlm_mfm_forward(self, frame_feats, frame_feats_mask, input_ids, text_attn_mask, segment_ids, probability_matrix,
        ...
        if self.visual_cls:
            mfm_multimodal_visual_emb = mfm_multimodal_visual_emb[:, 1:, :]
        loss_mfm = self.mfm_head(mfm_multimodal_visual_emb, mfm_labels, mfm_labels_index, frame_feats_mask)

        return loss_mlm, loss_mfm
...
if __name__ == '__main__':
    config = {'text_encoder_arch': 'bert', 'text_encoder_path': '../pretrained_model/roberta/',
              ...
              'ocr_mlm_probability': 0.1, 'tasks': 'itc,itm,mfm', 'mfm_probability': 0.15, 'pooling': '',
              'use_raw_image': True}
    model = MultiModal_Pretrain(config)

    image_feats = torch.randn((4, 10, 3, 224, 224))
    image_masks = torch.ones((4, 10))
