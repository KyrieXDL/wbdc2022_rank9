...
def prediction(model_stack, data_loader, device):
    ...
    with torch.no_grad():
        for step, batch in enumerate(data_loader):
            ...
            # logits = output1 * 0.45 + output2 * 0.55
            
            output1, output2, output3, output4 = output_list
            logits = (0.45 * output1 + 0.55 * output2) * 0.65 + (0.6 * output3 + output4 * 0.4) * 0.35
...
if __name__ == '__main__':
    ...
    # ========================== Title BERT =============================
    parser.add_argument('--bert_dir', type=str, default=root_path + 'opensource_models/pretrain_models/chinese-roberta-wwm-ext')
    parser.add_argument('--bert_dir2', type=str, default=root_path + 'opensource_models/pretrain_models/chinese-macbert-large')
