...
from transformers.models.bert.modeling_bert import BertPreTrainedModel, BertEmbeddings
from models.clip_vit import CLIPVisionModel
from transformers.models.clip.configuration_clip import CLIPVisionConfig
from models.BertModel1 import BertEncoder
...
class WXModel(nn.Module):
    def __init__(self, args, cfg, task=['mlm', 'mfm'], init_from_pretrain=True):
        ...
        # bertconfig
        bert_cfg = BertConfig.from_pretrained(os.path.join(args.bert_dir, 'config.json'))
        print(bert_cfg)
        ...
        self.bert = UniBertForMaskedLM(config=bert_cfg, add_pooling_layer=False, fusion_layer=self.fusion_layer)
        msg = self.bert.load_state_dict(bert_weight, strict=False)
        print(msg)
        #
        ...
        # else:
        #     # ????
        #     self.bert = UniBertForMaskedLM(bert_cfg, add_pooling_layer=True, fusion_layer=0)
    ...
    def forward(self, video_feature, video_mask, text_input_ids, text_mask, task=None):
        ...
        if 'mlm' in sample_task:
            ...
            lm_label = lm_label[:, 1:].to(text_input_ids.device)  # [SEP] ?? MASK ??? [SEP]
            return_mlm = True
...
class UniBertForMaskedLM(BertPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, fusion_layer=0):
        ...
        self.bert = UniBert(config, add_pooling_layer=add_pooling_layer, fusion_layer=fusion_layer)
...
class UniBert(BertPreTrainedModel):
    ...
    def forward(self, video_feature, video_mask, text_input_ids, text_mask, gather_index=None):
        embedding_text = self.embeddings(input_ids=text_input_ids)

        # reduce frame feature dimensions : 1536 -> 1024
        video_feature = self.video_fc(video_feature)
        video_feature = self.act(video_feature)
        embedding_video = self.video_embeddings(inputs_embeds=video_feature)
        ...
        return encoder_outputs, embedding_text, embedding_video
