...
class MultiModalDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.pseudo_zip_feat_path = args.pseudo_zip_feat_path
        ...
        # 是否使用伪标签
        if args.use_pseudo:
            with open(args.pseudo_ann_path, 'r', encoding='utf8') as f:
                pseudo_anns = json.load(f)
    ...
    def get_visual_feats(self, worker_id, idx: int) -> tuple:
        ...
        if self.handles[worker_id] is None:
            self.handles[worker_id] = zipfile.ZipFile(self.zip_feat_path, 'r')
        if self.handles_pseudo[worker_id] is None:
            self.handles_pseudo[worker_id] = zipfile.ZipFile(self.pseudo_zip_feat_path, 'r')

        if self.anns[idx]['labeled']:
            raw_feats = np.load(BytesIO(self.handles[worker_id].read(name=f'{vid}.npy')), allow_pickle=True)
        else:
            raw_feats = np.load(BytesIO(self.handles_pseudo[worker_id].read(name=f'{vid}.npy')), allow_pickle=True)
