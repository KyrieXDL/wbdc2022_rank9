...
def train_epoch(model, data_loader, loss_criterion, optimizer, scheduler, epoch, device, logger, args, momentum_model):
    ...
    for step, batch in enumerate(data_loader):
        ...
        if args.use_rdrop:
            ...
            outputs = model(double_frame_feat, double_frame_feat_mask, double_text_input_ids, double_text_segment_ids,
                            double_text_attention_mask)
        ...
        if args.use_multi_loss:
            output3, output4 = outputs[3], outputs[4]
            total_loss += loss_criterion(output3, label2) + loss_criterion(output4, label2)
        ...
        if args.use_distill:
            ...
            with torch.no_grad():
                output_lv2_m = momentum_model(frame_feat, frame_feat_mask, text_input_ids, text_segment_ids,
                                              text_attention_mask)
                logits_lv2_m = torch.softmax(output_lv2_m / 0.07, dim=1)
        ...
        if args.use_fgm:
            ...
            if args.use_fp16:
                with autocast():
                    output_adv = model(frame_feat, frame_feat_mask, text_input_ids, text_segment_ids,
                                       text_attention_mask)
                    loss_adv = loss_criterion(output_adv[0], label2)
            else:
                output_adv = model(frame_feat, frame_feat_mask, text_input_ids, text_segment_ids, text_attention_mask)
                loss_adv = loss_criterion(output_adv[0], label2)

            if args.use_contrastive:
                loss_adv_contras = output_adv[2] * 0
                loss_adv += loss_adv_contras
            ...
            for t in range(K):
                ...
                if args.use_fp16:
                    with autocast():
                        output_adv = \
                        model(frame_feat, frame_feat_mask, text_input_ids, text_segment_ids, text_attention_mask)[0]
...
def main(args):
    ...
    config = {'text_encoder_arch': args.text_encoder_arch, 'text_encoder_path': args.text_encoder_path,
              ...
              'frame_emb_type': args.frame_emb_type, 'use_multi_loss': args.use_multi_loss}
    if args.rank == 0:
        print(config)
    ...
    # if args.checkpoint:
    #     pretrained_names += ['vision_proj', 'text_proj', 'visual_encoder']
    ...
    if args.checkpoint:
        ...
        if args.use_momentum_ckpt:
            ckpt = {}
            for k, v in state_dict.items():
                if 'text_encoder_m' in k:
                    ckpt[k.replace('text_encoder_m', 'text_encoder')] = v
                    continue
                elif 'text_encoder' in k:
                    continue

                if 'visual_encoder_m' in k:
                    ckpt[k.replace('visual_encoder_m', 'visual_encoder')] = v
                    continue
    ...
    if args.phase == 'train':
        for epoch in range(start_epoch, args.epochs):
            ...
            if epoch >= 3:
                break
    elif args.phase == 'val' and val_size > 0:
        if val_size > 0:
            start_time = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_momentum_ckpt', action='store_true')
