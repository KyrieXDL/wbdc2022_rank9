...
class Wechat_Dataset(Dataset):
    def __init__(self, zip_frame_path='', zip_feat_path='', anno_path='',
                 labeled_zip_feat_path='', labeled_anno_path='',
                 phase='train', use_raw_image=False,
                 num_workers=1, max_frames=32, use_aug=False, args=None):
        ...
        self.labeled_zip_feat_path = labeled_zip_feat_path
        self.handles = [None for _ in range(num_workers)]
        self.labeled_handles = [None for _ in range(num_workers)]
        ...
        self.use_labeled_data = args.use_labeled_data
        ...
        if args.use_labeled_data:
            with open(labeled_anno_path, 'r', encoding='utf8') as f:
                labeled_anns = json.load(f)
            for a in labeled_anns:
                a['labeled'] = True
                data.append(a)
    ...
    def get_visual_feats(self, worker_id, idx):
        ...
        if self.labeled_handles[worker_id] is None and self.use_labeled_data:
            self.labeled_handles[worker_id] = zipfile.ZipFile(self.labeled_zip_feat_path, 'r')

        if not self.data[idx]['labeled']:
            raw_feats = np.load(BytesIO(self.handles[worker_id].read(name=f'{vid}.npy')), allow_pickle=True)
        else:
            raw_feats = np.load(BytesIO(self.labeled_handles[worker_id].read(name=f'{vid}.npy')), allow_pickle=True)
