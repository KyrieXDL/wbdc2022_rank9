...
from models.visual_encoder import VisualEncoder_Postnorm, VisualEncoder_Prenorm, TransformerHead
...
class MultiModal_Classifier(nn.Module):
    def __init__(self, config):
        ...
        self.use_multi_loss = config['use_multi_loss']
        ...
        if self.use_multi_loss:
            config = ViTConfig(hidden_size=768, num_hidden_layers=2, num_attention_heads=12)
            self.transformer_head = TransformerHead(config)
            self.image_classifier_head = nn.Linear(self.config['mm_embed_dim'], self.config['label2_nums'])
            self.text_classifier_head = nn.Linear(self.config['mm_embed_dim'], self.config['label2_nums'])
            self.lv2_classifier_head = nn.Linear(self.config['mm_embed_dim'], self.config['label2_nums'])
        else:
    ...
    def forward(self, frame_feats, frame_feats_mask, text_input_ids, text_segment_ids, text_attention_mask):
        ...
        if self.use_raw_image:
            visual_emb, visual_attn_mask = self.frame_encoder(visual_emb, visual_attn_mask,
                                                              emb_type=self.frame_emb_type)
        ...
        output1, output3, output4 = None, None, None
        if self.use_lv1:
            output1 = self.lv1_classifier_head(multimodal_emb)

        if self.use_multi_loss:
            cls_max = torch.max(torch.stack([multimodal_emb_image, multimodal_emb_text], dim=1), dim=1)[0]
            cls = torch.stack([cls_max, multimodal_emb_image, multimodal_emb_text, visual_emb, text_emb], dim=1)
            multi_cls = self.transformer_head(cls)[:, 0, :]
            output2 = self.lv2_classifier_head(multi_cls)
            output3 = self.image_classifier_head(multimodal_emb_image)
            output4 = self.text_classifier_head(multimodal_emb_text)
        else:
            output2 = self.lv2_classifier_head(multimodal_emb)

        if not self.training:
            return output2
        return (output2, output1, loss_itc, output3, output4)
